...
namespace ClientCredentialsDatabase.IdentityServer
{
    public class ResourceStore : IResourceStore
    {
        ...
        {
            var resource = await _session.Query<Database.Resource>().FirstOrDefaultAsync(x => x.Name == name);
            if (resource != default)
            {
        ...
        public async Task<IEnumerable<ApiResource>> FindApiResourcesByScopeAsync(IEnumerable<string> scopeNames)
        {
            ...
            {
                var scopes = await _session.Query<Database.Scope>().FirstOrDefaultAsync(x => x.Name == scope);
                if (scopes != default)
                {
        ...
        public Task<Resources> GetAllResourcesAsync()
        {
            ...
            {
                OfflineAccess = false,
                ApiResources = _session.Query<Database.Resource>().AsEnumerable().Select(x => ToApiResource(x)).ToList(),
                IdentityResources = Enumerable.Empty<IdentityResource>().ToList()
            });
        }

        private ApiResource ToApiResource(Database.Resource resource)
        {
